    ...
    startDynamicUpdate() {
      this.updateInterval = setInterval(() => {
        ...
        if (Math.random() > 0.7 && this.wordData.length < 15) {
          ...
          const newSkillPool = [
            // 传统技能
            { name: '机器学习', baseValue: 6500 },
            { name: '计算机视觉', baseValue: 6200 },
            { name: '数据分析', baseValue: 5800 },
            { name: '大数据', baseValue: 5500 },
            // 大模型技能（补充）
            { name: 'LLaMA', baseValue: 5100 },
            { name: 'ChatGLM', baseValue: 4800 },
            { name: 'Fine-tuning', baseValue: 4600 },
            { name: 'VectorDB', baseValue: 4300 }
          ]   

          // 过滤已存在的技能
          const availableSkills = newSkillPool.filter(skill => !this.wordData.some(item => item.name === skill.name)
          )   

...
export default {
  ...
  methods: {
    ...
    async fetchSkillData() {
      ...
      // 模拟数据：保留原有技能，新增Transformer/LLM等大模型相关技能
      sourceData = [
        { welfare_tag: '五险一金', word_count: 2481 },
        { welfare_tag: '定期体检', word_count: 1768 },
        { welfare_tag: '绩效奖金', word_count: 1603 },
        { welfare_tag: '年终奖金', word_count: 1490 },
        { welfare_tag: '餐饮补贴', word_count: 1452 },
        { welfare_tag: '专业培训', word_count: 1373 },
        { welfare_tag: '通讯补贴', word_count: 974 },
        { welfare_tag: '补充医疗保险', word_count: 843 },
        { welfare_tag: '员工旅游', word_count: 777 },
        { welfare_tag: '交通补贴', word_count: 538 },
        { welfare_tag: '弹性工作', word_count: 508 }
      ]

      // 格式转换：适配词云所需的{name, value}结构
      this.wordData = sourceData.map(item => ({
        name: item.welfare_tag,
    ...
    startDynamicUpdate() {
      this.updateInterval = setInterval(() => {
        ...
        if (Math.random() > 0.7 && this.wordData.length < 15) {
          ...
          const newSkillPool = [
            // 传统技能
            { name: '带薪年假', baseValue: 394 },
            { name: '补充公积金', baseValue: 394 }
          ]

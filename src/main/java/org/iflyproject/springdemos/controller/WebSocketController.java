...
import org.iflyproject.springdemos.config.Message;
...
public class WebSocketController {
    ...
    @MessageMapping("/api/arr")
    //在springmvc 中可以直接获得principal,principal 中包含当前用户的信息
    public void handleArr(Principal principal, Message messagePara) {
        String currentUserName = principal.getName();
        System.out.println(currentUserName);

        try {
            messagingTemplate.convertAndSendToUser(messagePara.getFrom(),
                    "/queue/notifications/arr",
                    messagePara);
        } catch (Exception e) {
            // 打印异常
            e.printStackTrace();
